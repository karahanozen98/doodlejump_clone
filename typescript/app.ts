...
class Game {
  ...
  private moveObjectsDown() {
    this.objects.forEach((obj: Step) => {
      // if object is a step move object down
      if (!(obj instanceof Player)) obj.setPosY(obj.getPosY() + this.gameSpeed);
  ...
  public increasescore() {
    this.score++;
    if (this.score >= 50) this.gameSpeed = 2.55;
    else if (this.score >= 40) this.gameSpeed = 2.35;
    else if (this.score >= 30) this.gameSpeed = 2.2;
    else if (this.score >= 20) this.gameSpeed = 2;
    else if (this.score >= 10) this.gameSpeed = 1.8;
  }
